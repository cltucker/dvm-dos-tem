...
// calculating daily 'atm' data for ONE year at yearly time-step - called
//   in 'RunCohort.cpp'
void Atmosphere::prepareDayDrivingData(const int & yrcount,
                                       const int & usedatmyr,
                                       const bool & changeclm,
  ...
  if (!changeclm) {
    for(int im =0; im<12; im++) {
      ...
      for (int id =0; id<31; id++) {
        ...
        abshd_d[im][id] = getAbsHumDeficit(svp_d[im][id], vap_d[im][id],
                                           tad1[id]);
    ...
    int iy= yrcount;
    iy = yrcount%usedatmyr; //this will reuse the first 'usedatmyr' period
                            //  of whole atm data set
...
void Atmosphere::precsplt(const float & tair,const float & prec,
                          float & snfl, float & rnfl) {
  /* *************************************************************
...
float Atmosphere::getAbsHumDeficit(const float & svp, const float &vp,
  ...
  ashd = fabs((Mw/R) * ((svp * 1000./(273.2 + dewpnt)) 
         - (vp *1000/(273.2 + ta))));
  return ashd;
};
